...
public class UserAgentHelperTest {
    ...
    public void userAgentGeneratedWithExpectedSdkAndJvmVersion() {
        ...
        assertEquals(helper.internalGetUserAgent(props, null), String.format("ask-java-model/%s Java/%s", TEST_SDK_VERSION, TEST_JVM_VERSION));
    }
    ...
    public void customUserAgentAppendedToEnd() {
        ...
        Properties props = mock(Properties.class);
        when(props.getProperty("java.version")).thenReturn(TEST_JVM_VERSION);
        assertEquals(helper.internalGetUserAgent(props, customUserAgent), String.format("ask-java-model/%s Java/%s foo/bar/baz", TEST_SDK_VERSION, TEST_JVM_VERSION));
    }

    @Test
    public void nullJvmPropertiesReturnsUnknownJvmVersion() {
        UserAgentHelper helper = UserAgentHelper.builder().withSdkVersion(TEST_SDK_VERSION).build();
        assertEquals(helper.internalGetUserAgent(null, null), String.format("ask-java-model/%s Java/UNKNOWN", TEST_SDK_VERSION));
    }
    ...
    public void unsetSdkVersionReturnsUnknownSdkVersion() {
        ...
        Properties props = mock(Properties.class);
        when(props.getProperty("java.version")).thenReturn(TEST_JVM_VERSION);
        assertEquals(helper.internalGetUserAgent(props, null), String.format("ask-java-model/UNKNOWN Java/%s", TEST_JVM_VERSION));
    }
